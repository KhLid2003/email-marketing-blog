...
export default function FeaturedPost({ title, excerpt, author, date, readTime, imageUrl, category }: FeaturedPostProps) {
  const articleUrl = `/article/${encodeURIComponent(title.toLowerCase().replace(/\s+/g, '-'))}`;
    ...
    <article className="relative bg-white rounded-xl overflow-hidden shadow-xl mb-12 hover:shadow-2xl transition-shadow duration-300">
      <div className="flex flex-col md:flex-row">
        ...
        <div className="md:w-1/2 p-8 flex flex-col justify-center">
          <span className="text-indigo-600 font-semibold mb-2 tracking-wide uppercase text-sm">Featured Post</span>
          <Link to={articleUrl}>
            <h1 className="text-4xl font-bold text-gray-800 mb-4 hover:text-indigo-600 cursor-pointer transition-colors">
              {title}
            </h1>
          </Link>
          <p className="text-gray-600 mb-6 text-lg leading-relaxed">{excerpt}</p>
          ...
          <Link to={articleUrl} className="inline-flex items-center gap-2 text-indigo-600 font-semibold hover:gap-3 transition-all group">
            Read Full Article 
